// Reads all leads, linkedin, analysis, and send_log data
...
const OUT_FILE  = path.join(DIR, 'dashboard.html');
...
const batchProg  = loadJSON(path.join(DIR, 'batch_progress.json')) || {};
...
console.log(`  Loaded ${leads.length} leads across ${industries.length} industries`);
...
<title>Catalyq — Lead Dashboard</title>
<style>
...
function exportCSV() {
  ...
  a.download = 'catalyq_' + new Date().toISOString().slice(0,10) + '.csv';
  a.click();
}
